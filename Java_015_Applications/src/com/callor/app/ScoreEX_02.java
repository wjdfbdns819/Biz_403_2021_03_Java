...
public class ScoreEX_02 {
	...
	public static void main(String[] args) {
		...
		scoreVO.setMath(inService.inputValue("영어", 0, 100));
		scoreVO.setMath(inService.inputValue("수학", 0, 100));
		scoreList.add(scoreVO);
		
		/*
		 * VO 객체에 데이터를 저장하고 List에 추가하려고 하면
		 * 	먼저 VO 객체를 다시 생성해 주어야 한다
		 */
		scoreVO = new ScoreVO(); // => 매우 중요함
		scoreVO.setNum("002");
		scoreVO.setName("성춘향");
		...
		//	전체 리스트의 데이터를 확인하는 용도로 재 정의 되어있다
		// 	이때 반드시 VO 클래스의 toString()도 재정의 되어있어야 한다
