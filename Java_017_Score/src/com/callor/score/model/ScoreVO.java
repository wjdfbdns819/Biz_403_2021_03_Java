...
public class ScoreVO {
	...
	/*
	 * VO 클래스를 모델링하고 : => 추상화 단계
	 *  변수를 private으로 선언하였다 : => 정보 은닉단계
	 * 	은닉된 변수에 접근하기 위하여 getter, setter method를 생성
	 * 
	 * getter와 setter를 생성하고 봤더니
	 * 	총점(total)과 평균(avg)는 점수가 입력되면
	 * 	항상 계산되어야 하는 항목이다
	 * 즉, 외부에서 이 변수에 값을 저장할 일이 별로 없을 것같다
	 * 		외부에서 total, avg 값을 요청하면
	 * 		그때 그때  계산하여 return을 하면 될것같다
	 * 
	 ...
	 * 
	 * 하지만 외부에서는 실제 코드가 어떻게 작동되는지 몰라도
	 * 	또는 관심 갖지 않아도 상관없이 getTotal() method는
	 * 	당연히 잘 계산된 총점결과를 알려준다는 것을 알고있다
	 * 
	 ...
	 * 	문자열로 만들어 return하는 코드로 다시 작성을 한다
