 ...
 * 
 *  객체지향 프로그램언어(java에서는)
 *  한개의 클래스(파일)내에서는 같은 이름의 함수(method)를 중복하여 선언할 수 있다
 *  	한개의 클래스에서 method를 중복선언할수 있다
 *  
 *  단, 조건이 있는데
 *  		매개변수가 달라야 한다
 *  	매개변수의 type, 개수, 순서가 다르면
 *  		같은 이름의 method를 중복선언할 수 있다	
 */
public class NumberServiceV1 {
	 ...
	 * 		두개의 변수를 선언하자
	 * 
	 * 두 수의 합을 계산하여 결과가 짝수이면 그 합을 return하고, => return 합; 형식의 명령이 있어야 한다
	 * 		그렇지 않으면 null을 return하라
	 * 			=> return null; 형식의 명령이 있어야 한다
	 * 
	 * return 값 형식의 명령이 있으려면 void가 아닌 return type을 지정해 주어야 한다
	 * 
	 * 	단순히 정수만 return을 하는 경우는 int type으로 하면되는데
	 * 		여기에서는 어떤 조건에 따라 null값을 return해야 하는 경우도 
	 * 		있기때문에 int type이 아니고 Integer type으로 설정해야 한다
	 */
	 ...
	 * 		addNum(int num1, int num2), addNum(long num1, long num2) 형식으로 
	 * 			method를 선언해야한다
	 * 
	 * 만약 addNum(실수, 실수) 방식으로 호출하려면
	 * 		addNum(float num1, float num2), addNum(double num1, double num2) 형식으로 
	 * 		method를 선언해야한다
	 * 
	...
	public Integer addNum(Integer intNum1, Integer intNum2 ) {
		Integer intNum = intNum1 + intNum2;
		if(intNum % 2 == 0) {
			return intNum;
