...
/*
 * 전통적인(구조적 프로그래밍) 방법 :
 * 	원칙적으로 같은 클래스에서 같은 이름의 method는 중복하여 작성할수 없다
 * 
 * inputValue1("새우깡");
 * inputValue2("새우깡",100);
 * inputValue3("새우깡"0, 1000);
 * 
 * inputValue2("새우깡", 0 , 1000); // => 매개변수 오류가 남
 * 
 *  // => 전통적인 방법은 일일이 순서를 붙여야 하는 번거로움이 있음
 * 
 ...
 * 
 * 객체지향 개발방법에서는
 * 	매개변수의 개수, 타입, 순서가 다르면 같은 이름의
 * 	method를 중복하여 작성할수 있다
 * 
 ...
 * 
 * 객체.inputValue("새우깡", 100); // 자동으로 2번 method가 연결, 호출이 됨
 * 객체.inputValue( 100 ,"새우깡" ); // 매개변수 오류가 남
 */
...
public class InputServiceV2 {
	...
	public Integer inputValue(String title,int start) {
		String msgTitle = String.format("%s(%d 이상의)", 
									title, start);
		while (true) {
	 ...
	 * 이미 만들어진 inputValue(String title) method와 연동하여
	 * 		사용하겠다
	 */
	public Integer inputValue(String title, int start, int end) {
		...
		String msgTitle = String.format("%s(%d ~ %d 범위의 )",
									title, start, end);
		while (true) {
			Integer intNum = this.inputValue(msgTitle);
			
			// this.inputValue(String title) method가
			// return 해준값이 null이 아니면 유효성 검사 수행
			if(intNum != null) {
				// 유효성 검사를 통과하지 못하면
				if(intNum < start || intNum > end) {
					// 메시지를 보여주고
					System.out.printf("입력 값은 %d ~ %d 까지 입력하세요\n"
							, start, end );
			...
			// 유효한 범위의 정수를 return 했으면 그 값이 담겨있다
			// 	그러면 그 값을 return 하라
			return intNum;
