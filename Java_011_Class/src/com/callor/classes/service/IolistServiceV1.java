 ...
 * 하지만 배열을 그 구조상 사용이 다소 불편할때가 많다
 * Java에서는 배열을 사용해야 하는 프로젝트에서 List 클래스를 사용한
 * 	객체를 생성하여 진행을 한다
 */
public class IolistServiceV1 {
	 ...
	 * 
	 * 낱개 데이터는 IolistVO 클래스형으로 사용하고
	 *   그 데이터들을 묶어서 List형으로 연산 수행을 하겠다 라는 선언
	 * 
	 * iolist를 private으로 선언한 이유
	 * 혹시 외부에서 iolist데이터에 접근하여 값을 저장하는 일을 
	 *   방지하기 위함이다
	 *  V1클래스를 상속하여 사용할 예정이라면 protected로 선언을 한다
	 */
	// TODO 맴버변수 선언하기
	...
	// public으로 선언된 method는 객체.method()형식으로 호출이 가능한 method
		// return type이 명확히 결정되지 않았을때는 void형으로 작성한다
	...
	public void printAllList() {
		...
		for(int i = 0 ; i < iolist.size(); i++) {
			...
			//this.printIolist(vo);
			this.printIoslist(iolist.get(i));
		}
		System.out.println("=".repeat(50));
	}
	
	public void printIoslist(IolistVO vo) {
		// TODO 1개 데이터 출력하기
