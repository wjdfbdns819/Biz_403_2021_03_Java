 ...
 * 이 5개의 변수는 반드시 ScoreVO 클래스를 객체로 선언한 후 사용할 수 있다
 */
public class ScoreVO {
	 ...
	 *  
	 *  한번 객체로 생성이 되면, 내부의 변수들은 절대 다른 데이터와 혼동되거나 하지 않는다
	 */
	...
	/*
	 * kor, eng, math 변수는 main() 메서드에서 사용자에 의해
	 * 	값을 저장하기 위한 변수이다
	 * 
	 * total, avg는 3과목의 점수를 사용하여 연산한 후
	 * 	연산 결과를 저장하기 위한 변수이다
	 * 
	 * kor, eng, math는 어디에서든지 저장, 읽기가 가능한 
	 * 	상태이어야 하므로 public으로 선언을 한다
	 * 
	 * 그런데 total, avg 변수도 public으로 선언을 해 두었더니
	 * 	실제 계산된 결과와 상관없이 임의의 값을 저장할수 있게
	 * 되었다
	 * 코드가 진행되는 과정에서 total에 임의의 값이 저장되면
	 * 평균값이 엉뚱하게 계산이되는 결과가 나타난다
	 * 
	 * 이런경우 total, avg 변수를 임의로 수정(저장)하지 못하도록
	 * 	막는 장치가 필요하다
	 * 
	 * VO 클래스의 멤버변수를 private으로 선언하면 다른곳에서 
	 * 	객체를 통해(점연결 연산자를 통해) 직접 접근이 불가능 해 진다
	 * 
	 * total, avg를 private으로 선언하면 누구도 직접 total, avg에 값을
	 * 	 저장할 수 없다
	 * 
