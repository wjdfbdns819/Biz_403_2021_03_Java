...
public class ScoreSeviceV1A {
	...
	// 클래스 영역에 Scanner 클래스를 사용하여 scan 객체를 선언
		...
		// 최초로 객체를 생성할때 필요한 개수를 매개변수로 전달하고
		// 전달받은 매개변수(members)의 값을 사용하여 배열을 만들자
		public ScoreSeviceV1A(int members) {
			// scan 객체를 생성(초기화)하기
			// 객체를 선언하고 생성하는 코드가 없으면 사용하는 과정에서 NullPointerException이
			// 발생한다
		...
		public void makeScores() {
			// 무한 반복문을 사용하여 국어점수가 0 ~ 100범위를 벗어나면
			// 메시지를 보여주고 다시 점수를 입력받도록 한다
			// 만약 정상적인 범위(0 ~ 100) 를 입력하면 반복문을 종료하고,
			// 다음 코드가 실행되도록 한다
			while(true) {
		...
		// public : 전체, 공용, 모두, 누구나
		// 객체.addNum() 형식으로 method를 호출하여 명령을 실행할수 있도록하는 Keyword
		public void addNum() {
			this.inputScore();
		...
		// method를 호출하여 명령을 실행할수 있도록 하는 Keyword
		// 객체.intputScore() 형식으로 사용이 불가능하다
		// 외부에 공개하지 않고, 현재 클래스의 코드에서만 호출할 수 있도록
		// 제한하는 keyword
		...
		// return type 이 void 형(type)일 경우는 return 명령문이 생략되거나 
		// return 명령문만 단독으로 사용할수있다
