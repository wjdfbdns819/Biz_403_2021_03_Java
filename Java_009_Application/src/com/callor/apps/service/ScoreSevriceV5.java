...
public class ScoreSevriceV5 {
	...
	Scanner scan;
	public ScoreSevriceV5(int members) {
	...
	public void makeScores() {
		...
		for(int i = 0 ; i <intKor.length ; i++) {
			...
			for(int j = 0 ; j < subject.length ; i ++) {
				...
				/*
				 * 입력업무중에 -1을 입력했을때는 입력업무를 중단하고
				 * main() 으로 되돌아가고싶다 이때 break를 사용하면 for()가 겹쳐있기 때문에
				 * 원하는 대로 코드가 작동되지 않는다
				 * 이럴 경우 return type이 void이지만 return명령을 실행하여
				 * method코드를 중단할수 있다
				 */
				if(scores[j] == -1) {
					return;
	...
	private Integer inputScore(String sub) {
		...
		while(true) {
			...
			// 점수입력을 하는 중에 -1 을 입력하면 입력하는 업무를 중단하자
			if(score == -1) {
				return -1;
