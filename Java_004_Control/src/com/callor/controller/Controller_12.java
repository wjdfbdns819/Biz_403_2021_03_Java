...
public class Controller_12 {
	...
	public static void main(String[] args) {
		/*
		 *  1 ~ 100까지 범위의 임의 정수 10개를 만들고 3의 배수인지 검사하여 
		 *  3의 배수이면 출력하고 수들의 합을 계산하여 출력하라
		 *  
		 *  변수의 선언 위치
		 */
		 ...
		 */
		for(int i = 0 ; i < 10 ; i++) {
			int num = rnd.nextInt(100) + 1; // 범위 
			if (num % 3 == 0)
			 {intSum3 += num;} // 3의 배수
			System.out.println(num +"는(은) 3의 배수");} 
		
			System.out.println("==================");
			System.out.printf("합계 : %d", intSum3);
			
			//for () {} 안에서 선언된 변수 num는 for() 명령이 끝나면 소멸된다
			// 변수 i 도 for() 명령이 끝나면 소멸된다
			// ex) System.out.println(num);
			 ...
			 * for() 반복문이 시작되기전에 변수를 반드시 선언하고 0으로 clear하는것이 좋다
			 */
	}
}
