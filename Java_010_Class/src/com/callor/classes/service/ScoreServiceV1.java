...
public class ScoreServiceV1 {
	 ...
	 * 
	 * class, method, 멤버변수를 선언할때 사용하는 Keyword
	 * 접근제한자는 사용하지 않아도 문법적인 오류가 발생하지 않는다
	 * 
	 * 객체지향 이론에 "정보은닉(감춘다, 제한적으로 접근하게 한다, 데이터를 보호한다)"
	 * 
	 ...
	 * 		클래스를 상속받게 하겠다 라는 전제하에 상속을 받은 클래스에서는
	 * 		자유롭게 접근 가능
	 * 
	 * 		부모 클래스와 상속받은 자식 클래스간에 공유하는 목적임
	 */
	
	/*
	 * 멤버 변수를 protected로 선언을 해두면 현재클래스(V1)를 상속받은 자식클래스에서
