...
public class Method_01 {
	...
	public static void main(String[] args) {
		 ...
		 * Random() 생성자 method를 호출하여 객채를 생성(초기화) 하기
		 * 객체를 선언만 하고 생성(초기화)을 하지않고,
		 * 		객체.method()를 호출하면 
		 * 		NullPointerException이 발생한다
		 * NullPointerException => 객체가 아직 실체가 없다는 메시지
		 *  
		 */
		 ...
		 * Scanner() 생성자 method를 호출하여 scan 객체를 생성한다
		 * Scanner() 생성자는 객체를 생성할때 System.in(키보드 입력 장치)를
		 * 		연결할수 있도록 매개 변수에 정보를 전달해주어야 한다
		 */
		 ...
		 * 조건에 일치하지 않는 else를 사용하여 표현할때는 딱 맞아 떨어지는
		 * 어떤 조건을 찾기가 어렵기 때문에 ~~가 아니다 라고 표현하는 것이 좋다
		 */
		 ...
		 * else이면 홀수이다 라고 표현할수 있지만 코드를 작성할때 조건으로
		 * 명시한 이외의 경우는 가급적 ~~이 아니다 라고 표현하자
		 */
		if(int1 % 2 == 0) {
